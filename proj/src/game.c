...
int (game_loop)() {
    ...
    //initAlienBullet(alienBullet_img);
                ...
                case HARDWARE:
                    if (msg.m_notify.interrupts & BIT(mouse_bit_no)) {
                        ...
                        if (currentByte == 1) {
                            ...
                            ///====================MENU====================
                            updateMouse(&pp, mouse); // updates mouse coordinates and
                            // rb_pressed variable according to the given packet
                            if (menuDisplay) {
                                button = getButton(mouse->x, mouse->y);
                                if (drawMenu(button)) {
                                    return 1;
                            ...
                            if (!gameOver && !menuDisplay && !instructionDisplay) {
                                ...
                                if (!alienBullet->active) {
                                    ...
                                    createAlienBullet(aliens[i].x + aliens[i].width / 2, aliens[i].y + aliens[i].height,
                                                      SHIP_BULLET_SPEED, alienBullet_img);
                                    drawAlienBullet();
                                } else {
                                    updateShipBulletPosition();
